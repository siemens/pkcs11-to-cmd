...
export P2C_CERT="$TEST_KEY_DIR/ec.pem"
export P2C_CMD=$SCRIPT_DIR/cmds/sign-cmd.sh

output=$(pkcs11-tool --slot-index 0 -s -p $PIN -m ECDSA-SHA256 --module $PKCS11_MODULE --input-file $TMPDATA --output-file $TMPSIG)

expect_succeeded_with "$output" "Signing with mechanism: CKM_ECDSA_SHA256"
