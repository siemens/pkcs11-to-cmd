...
export P2C_SLOT_CERT_0="$TEST_KEY_DIR/rsa.pem" # this is our CA certificate
...
ssh-keygen -D "$PKCS11_MODULE" > "$TEST_KEY_DIR/rsa.openssh.pub"

# sign the public ssh key with private key of the CA
# ssh-keygen uses $TEST_KEY_DIR/rsa.openssh.pub to find the matching private key in the PKCS#11 module
ssh-keygen -s "$TEST_KEY_DIR/rsa.openssh.pub" -D "$PKCS11_MODULE" -I test_user -n testuser -V +1h -z 1  "$KEY_NAME.pub"
